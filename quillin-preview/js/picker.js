...
import { buildVehicle, LINE } from './model.js?v=2e687300';
...
export function createPicker(opts) {
  ...
  /* WHEEL: the page's, not the car's. Same trap as the touch one above, on a
     pointer device: OrbitControls takes every wheel event on the canvas to
     zoom and cancels the scroll, so somebody scrolling down the damage page
     with the pointer resting over the stage, which is most of it, got the car
     spinning towards them instead of the list of windows. Found walking the
     flow as a customer. Stopped at the mount, in the capture phase, before
     the canvas ever sees it; nothing calls preventDefault, so the page
     scrolls. A trackpad pinch arrives as ctrlKey + wheel and still zooms. */
